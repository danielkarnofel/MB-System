...
[[nodiscard]] bool is_valid_record(const Ping &ping,
...
bool append_sounding_to_pointcloud(const Ping &ping,
    ...
    if (!is_valid_record(ping, sounding)) {
        set_error(
            error,
            "Sounding record contains a non-finite coordinate or timestamp");
        return false;
    }
    ...
    if (context->options.reject_flagged_beams &&
        !mb_beam_ok(sounding.beam_flag)) {
        ++stats.soundings_flagged;
        return true;
    }

    if (context->options.has_bounds &&
        !context->options.bounds.contains(
    ...
    if (!context->has_frame) {
        context->result.frame =
            coordinate_frame_from_origin(ping.sensor_position);
        context->has_frame = true;
    }

    context->result.points.push_back(
        make_collected_point(
            ping,
            sounding,
            context->result.frame));
...
bool register_read_array(int verbose,
                         void *mbio,
                         int memory_type,
    ...
    if (mb_register_array(
            verbose,
            mbio,
            memory_type,
            element_size,
            array,
            &error_code) == MB_SUCCESS) {
        return true;
    }

    set_error(
        error,
        "Unable to allocate MBIO " + std::string(array_name) +
            " array: " + mbio_error_message(verbose, error_code));
    return false;
}

bool register_read_arrays(int verbose,
                          void *mbio,
                          ReadArrays *arrays,
                          std::string *error) {
    return
        register_read_array(
            ...
            sizeof(char),
            reinterpret_cast<void **>(&arrays->beam_flags),
            "beam flag",
            error) &&
        register_read_array(
            verbose,
            ...
            MB_MEM_TYPE_BATHYMETRY,
            sizeof(double),
            reinterpret_cast<void **>(&arrays->bathymetry),
            "bathymetry",
            error) &&
        register_read_array(
            verbose,
            ...
            sizeof(double),
            reinterpret_cast<void **>(&arrays->amplitude),
            "amplitude",
            error) &&
        register_read_array(
            verbose,
            ...
            MB_MEM_TYPE_BATHYMETRY,
            sizeof(double),
            reinterpret_cast<void **>(&arrays->bathymetry_across_track),
            "bathymetry across-track",
            error) &&
        register_read_array(
            verbose,
            mbio,
            MB_MEM_TYPE_BATHYMETRY,
            sizeof(double),
            reinterpret_cast<void **>(&arrays->bathymetry_along_track),
        ...
        register_read_array(
            ...
            sizeof(double),
            reinterpret_cast<void **>(&arrays->sidescan),
            "sidescan",
            error) &&
        register_read_array(
            verbose,
            ...
            MB_MEM_TYPE_SIDESCAN,
            sizeof(double),
            reinterpret_cast<void **>(&arrays->sidescan_across_track),
            "sidescan across-track",
            error) &&
        register_read_array(
            verbose,
            mbio,
            MB_MEM_TYPE_SIDESCAN,
            sizeof(double),
            reinterpret_cast<void **>(&arrays->sidescan_along_track),
            "sidescan along-track",
            error);
}

bool process_ping(int verbose,
                  double timestamp,
                  double navigation_longitude,
                  double navigation_latitude,
                  double heading,
                  double sensor_depth,
                  int beam_count,
                  const ReadArrays &arrays,
                  ReadContext *context,
                  std::string *error) {
    if (!std::isfinite(timestamp) ||
        !std::isfinite(navigation_longitude) ||
        !std::isfinite(navigation_latitude) ||
        !std::isfinite(heading) ||
        !std::isfinite(sensor_depth)) {
    ...
    if (mb_coor_scale(
            verbose,
            navigation_latitude,
            &meters_to_degrees_longitude,
            &meters_to_degrees_latitude) != MB_SUCCESS) {
        set_error(
            error,
            "Unable to calculate coordinate scale: " +
                mbio_error_message(verbose, error_code));
        return false;
    }
    ...
    for (int beam_index = 0; beam_index < beam_count; ++beam_index) {
        const double across_track =
            arrays.bathymetry_across_track[beam_index];
        const double along_track =
            arrays.bathymetry_along_track[beam_index];

        Sounding sounding;
        sounding.position.longitude =
            navigation_longitude +
            heading_y * meters_to_degrees_longitude * across_track +
            heading_x * meters_to_degrees_longitude * along_track;
        sounding.position.latitude =
            navigation_latitude -
            heading_x * meters_to_degrees_latitude * across_track +
            heading_y * meters_to_degrees_latitude * along_track;
        ...
        if (!append_sounding_to_pointcloud(
                ping,
                sounding,
                context,
                error)) {
...
bool read_swath_file(const char *file,
                     int format,
                     int alternate_navigation_status,
                     char *alternate_navigation_path,
                     ReadContext *context,
                     std::string *error) {
    ...
    if (mb_read_init_altnav(
        ...
        set_error(
            error,
            "Unable to initialize swath file '" + std::string(file) +
                "': " + mbio_error_message(verbose, error_code));
        return false;
    }
    ...
    while (true) {
        ...
        if (status != MB_SUCCESS) {
            ...
            set_error(
                error,
                "Error reading swath file '" + std::string(file) +
                    "': " + mbio_error_message(verbose, error_code));
            success = false;
            break;
        }
    ...
    if (mb_close(verbose, &mbio, &close_error) != MB_SUCCESS && success) {
        set_error(
            error,
            "Unable to close swath file '" + std::string(file) +
                "': " + mbio_error_message(verbose, close_error));
        success = false;
    }
...
bool GeographicBounds::contains(double lon, double lat) const {
    const bool latitude_in_bounds =
        lat >= degrees_S && lat <= degrees_N;
    const bool longitude_in_bounds =
        degrees_W <= degrees_E
...
CoordinateFrame coordinate_frame_from_origin(
    ...
    frame.meters_per_degree_lat =
        earth_radius_meters * degrees_to_radians;
    frame.meters_per_degree_lon =
        earth_radius_meters *
        std::cos(origin.latitude * degrees_to_radians) *
        degrees_to_radians;
    return frame;
}

Vec3 geodetic_to_local(const GeodeticPosition &position,
                       const CoordinateFrame &frame) {
    return Vec3(
        (position.longitude - frame.origin.longitude) *
            frame.meters_per_degree_lon,
        (position.latitude - frame.origin.latitude) *
            frame.meters_per_degree_lat,
        position.elevation - frame.origin.elevation);
}

bool read_datalist(const std::filesystem::path &path,
                   const DatalistReaderOptions &options,
                   DatalistReadResult *result,
                   std::string *error) {
    if (error != nullptr) {
    ...
    if (mb_datalist_open(
        ...
        set_error(
            error,
            "Unable to open datalist '" + datalist_path +
                "': " + mbio_error_message(verbose, error_code));
        return false;
    }
    ...
    while (true) {
        ...
        if (status != MB_SUCCESS) {
            if (error_code != MB_ERROR_EOF &&
                ...
                set_error(
                    error,
                    "Error reading datalist '" + datalist_path +
                        "': " +
                        mbio_error_message(verbose, error_code));
                success = false;
            }
        ...
        char *file =
            processed_status == MB_PROCESSED_USE
                ? processed_path
                : raw_path;
    ...
    int close_error = MB_ERROR_NO_ERROR;
    if (mb_datalist_close(
            verbose,
            &datalist,
            &close_error) != MB_SUCCESS && success) {
        set_error(
            error,
            "Unable to close datalist '" + datalist_path +
                "': " + mbio_error_message(verbose, close_error));
        success = false;
    }
    ...
    if (read_result.points.empty()) {
        set_error(
            error,
            "Datalist produced no accepted bathymetry soundings: " +
                datalist_path);
        return false;
    }
