...
inline std::vector<OrientedPoint> estimate_normals_PCA(const std::vector<Vec3> &points, int k) {
	...
	for (std::size_t i = 0; i < points.size(); i++) {
		const std::vector<KDTree::Neighbor> neighbors = tree.k_nearest(points[i], k, i);
		...
		for (std::size_t j = 0; j < neighbors.size(); j++) {
			local_center = add(local_center, points[neighbors[j].index]);
		}
		local_center = multiply(local_center, 1.0 / neighbors.size());

		Mat3 covariance;
		for (std::size_t j = 0; j < neighbors.size(); j++) {
			const Vec3 d = subtract(points[neighbors[j].index], local_center);
			covariance.add_outer_product(d);
		}
		covariance = covariance * (1.0 / neighbors.size());

		const EigenDecomposition eig = covariance.eigen_decomposition_symmetric();
		...
		oriented_points.push_back({points[i], normal, eig.values.x, eig.values.y, eig.values.z});
	}
